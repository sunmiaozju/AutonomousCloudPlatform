...
public class    RentController {
    ...
    public ResultObj checkCustomerExist(RentVo rentVo){
        Customer customer = customerService.queryCustomerByPhoneNumber(rentVo.getPhone());
        if (null!=customer){
    ...
    public ResultObj saveRent(RentVo rentVo){
        Customer customer = customerService.queryCustomerByPhoneNumber(rentVo.getPhone());
        if (null==customer){
            return ResultObj.RENT_ERROR_NO_PHONE;
        }
        try {
            //设置创建时间
    ...
    public ResultObj updateRent(RentVo rentVo){
        Customer customer = customerService.queryCustomerByPhoneNumber(rentVo.getPhone());
        if (null == customer){
            return ResultObj.UPDATE_ERROR_NO_PHONE;
        }
        try {
            //修改
    ...
    public DataGridView loadAllRent(RentVo rentVo){
        log.info("DataGridView : " + JSON.toJSONString(this.rentService.queryAllRent(rentVo)));
