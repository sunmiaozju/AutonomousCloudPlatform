    ...
    <div class="layui-form-item">
        <div class="layui-inline">
            ...
            <div class="layui-input-inline" style="padding: 5px;width: 280px">
                <input type="text" name="search_rentid" id="search_rentid"
                       class="layui-input layui-input-inline"
                       placeholder="请输入订单号" style="height: 30px;border-radius: 10px;width: 280px">
            </div>
...
<div id="content" style="display: none">
    ...
    <form class="layui-form" method="post" id="checkFrm" lay-filter="checkFrm">
        <div class="layui-form-item">
            <div class="layui-inline">
                <label class="layui-form-label" >结束单号:</label>
                <div class="layui-input-inline" style="padding: 5px;width: 280px">
                    <input type="text" name="checkid" readonly="readonly"
                           class="layui-input layui-input-inline"
                           placeholder="请输入结束单号" style="height: 30px;border-radius: 10px;width: 280px">
                </div>
            </div>
            <div class="layui-inline">
                <label class="layui-form-label" style="width: 100px;">实际到达时间:</label>
                <div class="layui-input-inline" style="padding: 5px;width: 280px">
                    <input type="text" name="checkdate" id="checkdate" required  lay-verify="required"
                           class="layui-input layui-input-inline"
                           placeholder="请输入结束时间" style="height: 30px;border-radius: 10px;width: 280px">
                </div>
            </div>
        </div>
        <div class="layui-form-item">
...
<%--            </div>--%>
        ...
        <div class="layui-form-item layui-form-text">
            <label class="layui-form-label" >体验评价:</label>
            <div class="layui-input-block">
                <textarea name="checkdesc" placeholder="请输入乘坐体验评价" required  class="layui-textarea"></textarea>
            </div>
        </div>
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate'], function () {
        ...
        $("#doSearch").click(function () {
            ...
            $.post("${yeqifu}/check/checkRentExist.action", {rentid: rentid}, function (obj) {
                if (obj === "") { //订单号不存在，返回值为null
                    layer.msg("您输入的订单号不存在，请更正后再查询");
                    //隐藏数据表格
                    $("#content").hide();
                } else {
                    if (obj.rentflag == 1) {
                        layer.msg("您所输入的订单已完成，请输入其他订单号");
                        $("#content").hide();
                    } else {
                        //初始化表单数据和卡片面板数据
        ...
        function initCheckFormData(rentid) {
            $.post("${yeqifu}/check/initCheckFormData.action",{rentid:rentid},function (obj) {
                ...
                $("#rent_rentid").html("订单号: "+rent.rentid);
                $("#rent_price").html("订价格: "+rent.price);
                ...
                $("#car_rentprice").html("订价格: "+car.rentprice);
                $("#car_deposit").html("订押金: "+car.deposit);
